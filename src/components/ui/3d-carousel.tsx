...
const transitionOverlay = { duration: 0.8, ease: [0.22, 1, 0.36, 1] }
      ...
      >
        <motion.div
          ...
          onDragEnd={(_, info) =>
            isCarouselActive &&
            controls.start({
              rotateY: rotation.get() + info.velocity.x * 0.025,
              transition: {
                type: "spring",
                stiffness: 100,
                damping: 30,
...
function ThreeDPhotoCarousel() {
  ...
  useEffect(() => {
    if (activeImg && carouselRef.current) {
        ...
        {activeImg && (
          ...
          >
            <motion.img
              ...
              src={activeImg}
              className="w-auto h-auto max-w-[90vw] max-h-[90vh] sm:max-w-2xl md:max-w-3xl lg:max-w-4xl md:max-h-[80vh] rounded-lg shadow-lg"
              initial={{ scale: 0.5 }}
              animate={{ scale: 1 }}
